...
def test_allows_up_to_the_limit_then_refuses(server: fakeredis.FakeServer) -> None:
    ...
    for _ in range(LIMIT):
        assert subject.check("ip:1.2.3.4").allowed is True

    refused = subject.check("ip:1.2.3.4")
...
def test_two_replicas_share_one_budget(server: fakeredis.FakeServer) -> None:
    ...
    assert replica_a.check("ip:9.9.9.9").allowed is True
    assert replica_b.check("ip:9.9.9.9").allowed is True
    assert replica_a.check("ip:9.9.9.9").allowed is True

    assert replica_b.check("ip:9.9.9.9").allowed is False
...
def test_distinct_keys_have_independent_budgets(server: fakeredis.FakeServer) -> None:
    ...
    for _ in range(LIMIT):
        assert subject.check("ip:1.1.1.1").allowed is True

    assert subject.check("ip:1.1.1.1").allowed is False
    assert subject.check("ip:2.2.2.2").allowed is True
...
def test_a_refused_request_does_not_consume_a_slot(
    ...
    subject = limiter(server)
    for _ in range(LIMIT):
        subject.check("ip:3.3.3.3")

    first_refusal = subject.check("ip:3.3.3.3")
    second_refusal = subject.check("ip:3.3.3.3")
...
def test_reset_clears_shared_state(server: fakeredis.FakeServer) -> None:
    subject = limiter(server)
    for _ in range(LIMIT):
        subject.check("ip:6.6.6.6")
    assert subject.check("ip:6.6.6.6").allowed is False

    subject.reset()

    assert subject.check("ip:6.6.6.6").allowed is True
...
def test_a_redis_outage_fails_open_onto_the_in_process_limiter() -> None:
    ...
    for _ in range(LIMIT):
        assert subject.check("ip:7.7.7.7").allowed is True

    assert subject.check("ip:7.7.7.7").allowed is False
...
def test_the_fallback_still_enforces_a_limit_rather_than_allowing_everything() -> None:
    ...
    assert subject.check("ip:8.8.8.8").allowed is True
    assert subject.check("ip:8.8.8.8").allowed is False
...
def test_reset_is_survivable_when_redis_is_unreachable() -> None:
    ...
    assert subject.check("ip:1.0.0.1").allowed is True
